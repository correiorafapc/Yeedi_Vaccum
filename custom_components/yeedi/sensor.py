...
from .const import ERROR_CODE_MAP #RPCO
...
async def async_setup_entry(
    ...
    entities.extend(
        EcovacsErrorDescriptionSensor(device, capability)
        for device in controller.devices
        if (capability := device.capabilities.error)
    )
...
def _normalize_error_code(code: Any) -> Any:
    """Normalize error codes so dict keys like 20003 work even if code is '20003'."""
    try:
        return int(code)
...
class EcovacsErrorDescriptionSensor(
    EcovacsEntity[CapabilityEvent[ErrorEvent]],
    SensorEntity,
):
    """Error description sensor (mapped from error code)."""
    ...
    async def async_added_to_hass(self) -> None:
        ...
        async def on_event(event: ErrorEvent) -> None:
            code_norm = _normalize_error_code(event.code)

            # Prefer your mapping; fall back to event.description; then final fallback.
            mapped = ERROR_CODE_MAP.get(code_norm)
            description = mapped or event.description or "Unknown error"

            self._attr_native_value = description
            self._attr_extra_state_attributes = {
                "code": event.code,
                "mapped": mapped is not None,
                CONF_DESCRIPTION: event.description,  # keep original vendor text as attribute
            }
