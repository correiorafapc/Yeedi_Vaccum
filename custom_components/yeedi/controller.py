...
from .yeedi_devices import YEEDI_DEVICES, register_yeedi_devices
...
class EcovacsController:
    ...
    async def initialize(self) -> None:
        """Init controller."""
        try:
            # Path 1: inject Yeedi profiles into the deebot_client hardware
            # registry (works on deebot_client >= 4.0 with hardware.deebot).
            # On older/restructured versions this is a no-op and we fall back
            # to Path 2 below.
            register_yeedi_devices()

            devices = await self._api_client.get_devices()
            credentials = await self._authenticator.authenticate()

            # Collect all MQTT device infos: recognised ones from the API
            # response plus any Yeedi devices rescued from not_supported.
            mqtt_device_infos = list(devices.mqtt)

            # Path 2: for any not_supported device whose class string is in
            # YEEDI_DEVICES, build a DeviceInfo manually and treat it as an
            # MQTT device.  This handles deebot_client versions that lack the
            # hardware.deebot registry (so register_yeedi_devices was a no-op).
            try:
                from deebot_client.models import DeviceInfo as _DeviceInfo
                for raw in devices.not_supported:
                    class_string = raw.get("class", "")
                    if class_string in YEEDI_DEVICES:
                        device_name = raw.get(
                            "deviceName",
                            raw.get("nick", raw.get("class", "Unknown")),
                        )
                        _LOGGER.debug(
                            "Yeedi device '%s' (%s) rescued from not_supported",
                            device_name,
                            class_string,
                        )
                        mqtt_device_infos.append(
                            _DeviceInfo(raw, YEEDI_DEVICES[class_string])
                        )
                    else:
                        device_name = raw.get(
                            "deviceName",
                ...
                # DeviceInfo not importable — log remaining not_supported as-is
                for device_config in devices.not_supported:
                    device_name = device_config.get(
                        "deviceName",
                        device_config.get("nick", device_config.get("class", "Unknown")),
                    )
                    _LOGGER.warning(
                        (
                            'Device "%s" not supported. More information at '
            ...
            if mqtt_device_infos:
                mqtt = await self._get_mqtt_client()
                mqtt_devices = [
                    Device(info, self._authenticator) for info in mqtt_device_infos
                ]
